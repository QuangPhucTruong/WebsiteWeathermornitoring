...
btnOff1.onclick = function() {
    ...
    database.ref("/My_device").update({"LED": 0 });
}
btnOff2.offclick = function() {
    document.getElementById("denId_02").src = "./img/airoff.png"
    database.ref("/Thiet bi cua toi").update({
        "led02": "0"
    });
}
btnOff3.offclick = function() {
...
database.ref("/Thoi tiet hom nay/gio").on("value", function(snapshot) {
    if (snapshot.exists()) {
...
database.ref("/Thiet bi cua toi/led02").on("value", function(snapshot2) {
    if (snapshot2.exists()) {
        ...
        if (ss2 == 1)
            document.getElementById("denId_02").src = "./img/airon.png"
        else
            document.getElementById("denId_02").src = "./img/airoff.png"
    } else
        console.log("No data available!")
